						...
						<div class="card shadow">
							...
							<div class="card-body">
								<form action="<?= base_url('pengeluaran/proses_tambah') ?>" id="form-tambah" method="POST">
									...
									<hr> -->
									<div class="form-row">
										<input type="hidden" name="no_keluar" value="TR<?= time() ?>" readonly class="form-control">
										
										<input type="hidden" name="kode_petugas" value="<?= $this->session->login['kode'] ?>" readonly class="form-control">
										
										<input type="hidden" name="nama_petugas" value="<?= $this->session->login['nama'] ?>" readonly class="form-control">
										...
										<div class="col-md-4">
											...
											<div class="form-row">
												<div class="form-group col-10">
													<label for="nama_customer">WH Tujuan</label>
													<select name="nama_customer" id="nama_customer" class="form-control">
														<option value="">Pilih WH Tujuan</option>
			...
			$(document).on('click', '#tambah', function(e){
				...
				if(parseInt($('input[name="max_hidden"]').val()) <= parseInt(data_keranjang.jumlah)) {
					...
					$.ajax({
						...
						data: data_keranjang,
						success: function(data){
							if($('select[name="kode_barang"]').val() == data_keranjang.kode_barang) $('option[value="' + data_keranjang.kode_barang + '"]').hide()
							reset()
